...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu,
                     optimizer="adamw", poly_power=1.0, start_warmup_step=0, epsilon=1e-6,
                     colocate_gradients_with_ops=False, return_optimizer=False):
    ...
    if optimizer == "adamw":
        ...
        optimizer = lamb_optimizer.LAMBOptimizer(
            ...
            epsilon=epsilon,
            exclude_from_weight_decay=["LayerNorm", "layer_norm", "bias"])
    elif optimizer == 'adamw-dist':
        tf.logging.info("using lamb")
        optimizer = AdamWeightDecayDistributeOptimizer(
            learning_rate=learning_rate,
            weight_decay_rate=0.01,
            beta_1=0.9,
            beta_2=0.999,
...
class AdamWeightDecayDistributeOptimizer(tf.train.Optimizer):
    """A basic Adam optimizer that includes "correct" L2 weight decay."""
    ...
    def __init__(self,
                 ...
                 name="AdamWeightDecayOptimizer"):
        """Constructs a AdamWeightDecayOptimizer."""
        super(AdamWeightDecayDistributeOptimizer, self).__init__(False, name)
